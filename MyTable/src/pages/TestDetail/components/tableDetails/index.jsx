...
import { Descriptions, Card, Button, Input, Result, Tooltip } from 'antd';
...
const RenderEquip = (props) => {
    const { equipData, state, ip, typeId, port } = props
    ...
    return (
        <div className={styles.table}>
            <Card >
                ...
                <Descriptions title={<TitleComponent ip={ip} state={state} handleSingleEnd={handleSingleEnd} allowToStop={allowToStop} />} bordered column={2} size="small">
                    ...
                    </Descriptions.Item>
                    {
                        Object.keys(functionParams).map(key => {
                            const functions = equipData.testTarget.functionParam
                            const hasWater = functions.hasOwnProperty(functionParams[key])
                            if (!hasWater) {
                                return <></>
                            }
                            return (
                                <Descriptions.Item label={key}>
                                    <StateIcon state={functions[functionParams[key]]}>
                                        {functions[equipKeyDict[key]]}
                                    </StateIcon>
                                </Descriptions.Item>
                            )
                        })
                    ...
                    <Descriptions.Item label="设备属性" span={2}>
                        <Tooltip title={JSON.stringify(equipData.testTarget.readParams)}>
                            <Input value={JSON.stringify(equipData.testTarget.readParams)} />
                        </Tooltip>

                    </Descriptions.Item>
                    <Descriptions.Item label="设备写入属性" span={2}>
                        <Tooltip title={JSON.stringify(equipData.testTarget.writeParams)}>
                            <Input value={JSON.stringify(equipData.testTarget.writeParams)} />
                        </Tooltip>
                    </Descriptions.Item>
...
const TableDetails = props => {
    ...
    if (typeof props.equipData === 'object') {
        return <RenderEquip equipData={equipData} state={state} ip={ip} port={port} typeId={typeId} />
    }
    return (
        <div className={styles.table}>
            <Card>
