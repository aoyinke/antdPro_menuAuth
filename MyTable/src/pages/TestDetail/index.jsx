...
import { Button, Card, Descriptions, message, Form, Modal, Input } from 'antd';

import { connect } from 'umi';
import styles from './style.less';
import TableDetails from './components/tableDetails';
import { getTest, endTest, startPrintTest, startToTest } from './service';
    ...
    <div className={styles.equiTestBar}>
      {Object.keys(currentTestEquipList).map((ip) => {
        return (
          <TableDetails
            ip={ip}
            equipData={currentTestEquipList[ip]}
            port={params.port}
...
const TestDetail = (props) => {
  ...
  useEffect(() => {
    const urlPort = props.location.query.port;
    const urlTypeId = props.location.query.typeId;
    dispatch({
  ...
  const startTest = async () => {
    ...
    try {
      const params = { typeId, port };
      const res = await startToTest(params);
      if (res.code === 200) {
        hide();
  ...
  const onConfirmStartToPrint = async () => {
    const { imei, imsi, macId, model } = form.getFieldValue();
    ...
    const params = { imei, imsi, macId, typeId, model };
    try {
      const res = await startPrintTest(params);
      if (res.code === 200) {
        hide();
  ...
  return (
    <PageHeaderWrapper title={false}>
      ...
      >
        ...
        >
          ...
          <div className={styles.equiBtn}>
            ...
            <Button type="primary" style={{ margin: '15px' }} onClick={startTest}>
              开始测试
            </Button>
            <Button
              type="primary"
              danger
              disabled={!readyStartToEnd}
              onClick={async () => {
                clearInterval(intervalTest);
      ...
      <BeginToRenderEqui
        params={{ ips: props.location.query.ipList, port, typeId }}
        readyToRender={readStartTogetTest}
        handleAllowToEndAll={handleAllowToEndAll}
      />
      <Modal
        title="输入测试信息"
        ...
        footer={null}
      >
        <Form onFinish={onConfirmStartToPrint} form={form}>
          <Form.Item label="imei" name="imei" rules={[{ required: true, message: '请输入imei!' }]}>
            <Input />
          </Form.Item>

          <Form.Item label="imsi" name="imsi" rules={[{ required: true, message: '请输入imsi!' }]}>
            <Input />
          </Form.Item>
          <Form.Item
            label="macId"
          ...
          <div className={styles.buttonPois}>
            <Button type="primary" onClick={onConfirmStartToPrint}>
              确认
            </Button>
          </div>
        </Form>
